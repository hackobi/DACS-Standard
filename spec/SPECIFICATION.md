...
(VP-C1) A VerifyResult for (scheme, identifier, recipeVersion) MAY be reused while `now ≤ min(VerifyResult.validUntil ?? ∞, verifiedAt + defaultMaxAgeSec × 1000)`, where `defaultMaxAgeSec` is from the recipe at that `recipeVersion` — the SAME effective `min`-clamp window the §6.3.2 freshness gate uses (the reuse and freshness rules therefore agree; reuse is never permitted past the freshness window). (VP-C2) Reuse MUST update the consuming session’s record to reference the cached VerifyResult. (VP-C3) Reuse MUST NOT bypass freshness requirements declared by the listing’s ClaimRequirement.maxAge.
...
- **Caching semantics (VP-C1..VP-C3).** Reuse within the effective `min(validUntil, verifiedAt + defaultMaxAgeSec)` window (defaultMaxAgeSec from the recipe at the result's recipeVersion — same window as the §6.3.2 freshness gate); record-update on reuse; maxAge override of recipe default.
