...
type IdentityBundle = {
  ...
  identityTier?: "institutional" | "verified" | "self-declared"  // derived trust tier; readers recompute from verified claims and ignore self-asserted values
...
**Identity tier derivation.** `identityTier` is an optional production trust signal on IdentityBundle. It is not an authority assertion and MUST NOT be trusted merely because it appears in the bundle. A conforming reader MUST recompute it deterministically from the resolved claims it treats as verified and MUST ignore a self-asserted value that disagrees with the recomputed value. Only verified claims count toward identityTier. Derivation is: (1) if `claims[]` contains a verified authority-issued institutional claim (`lei:`, `finra-crd:`, `sam-uei:`, `fedramp:`, `cmmc:`, `naics:`, or an equivalent steward-registered institutional scheme), the tier is `"institutional"`; (2) else if `claims[]` contains a verified operational-control claim (`domain:`, ACME/domain-control via the DACS-2 recipe registry, TLSNotary/zkTLS-backed domain or service-control proof, `erc8004:` ownership with a passing VerifyResult, or an equivalent steward-registered verified scheme), the tier is `"verified"`; (3) otherwise the tier is `"self-declared"`. Institutional takes precedence over verified. Unknown schemes and claims with missing, stale, failing, indeterminate, errored, or unresolved `verifiedBy` are treated as unverified for this derivation. Two conformant readers given the same bundle and the same resolved VerifyResults MUST derive the same tier.
...
type ReputationDerivation = {
  ...
  suspiciousPatternFlags?: string[]            // advisory implementation-defined behavioral flags; never changes core metrics
...
derive(party, bundles, windowStart, windowEnd):
  ...
  suspiciousPatternFlags := implementation_defined_flags(reconciled, outcomes, metrics)  // optional advisory signal; does not feed back into the metrics above

  return ReputationDerivation with computed metrics and optional suspiciousPatternFlags
```
...
**Suspicious pattern flags (advisory).** `suspiciousPatternFlags` is an optional string-array production trust signal on ReputationDerivation / ReputationRecord surfaces. The field shape is standardised, but the detection algorithms, thresholds, and vocabulary are implementation-defined. It MUST NOT change core reputation derivation, MUST NOT alter `bundleCount`, MUST NOT alter the metrics computed above, and MUST NOT automatically convert a passing DACS-5 derivation into a failing one. Consumers MAY use the flags for UI warnings, risk policy, or listing-side gating when the listing explicitly declares such a policy. The vocabulary is open; suggested initial flag names are `high-volume-single-counterparty-cluster`, `unusually-low-outcome-variance`, and `rapid-bundle-growth-with-zero-disputes`. Reputation consumers SHOULD treat very low `bundleCount` records cautiously unless additional identity or verification signals are present.
...
**DACS-X interface seam (non-normative pack).** To prevent the dispute layer and settlement lane from drifting while DACS-X is designed, this repository carries a provisional interface pack for shared fixtures and verifiers. The pack pins a minimal `DisputeOutcome` artifact shape for §10.4.3 bundle divergence and HTLC-9 close-out testing: `outcomeVersion`, `jobId`, `disputeKind`, `subjectBundleRefs`, `subjectEvidenceRef`, `decision`, `rationale`, optional `disclosureRefs`, and optional `settlementAmendment`. When an HTLC-9 `dest-revealed-source-unclaimed` state is later proven resolved by the payee source-side `htlc-claim`, the DACS-X seam models the close-out as a `SettlementAmendment` with `amendmentType: "correction"`, no `refundAmount`, and a reason such as `dacsx-htlc9-source-claim-confirmed`. This is a verifier/fixture contract for interoperability testing, not a full dispute-resolution standard and not a change to the v0.1 ST-8 success path.
