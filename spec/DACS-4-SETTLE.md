...
- **Specified, not yet reference-backed** — `pay-ap2` (§9.5.6) and `pay-x402` (§9.5.7): their handler procedure, registry entries, and evidence shape are defined, but in v0.1 there is **no live settlement path and no §14 conformance vector** for either. `pay-ap2` is additionally `operator_gated` (it requires AP2 provider onboarding — Visa Direct / Mastercard Send / Stripe PaymentIntents), and AP2 itself was donated to the FIDO Alliance only in April 2026. Their v0.1 registry entries therefore declare a non-`live` `availability` (`operator_gated` or `mocked`), and orchestrators MUST NOT treat them as `live` (RAV-R1). Bringing both to reference-backed status (a live path plus conformance vectors) is roadmap work.
