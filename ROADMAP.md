...
| `pay-ap2` / `pay-x402` reference-backing | Specified; not yet reference-backed | Both rails are **spec-complete** in v0.1 (handler procedure, registry entries, evidence shape — §9.5.6 / §9.5.7) but have **no live settlement path and no §14 conformance vector**, so they do not meet the v0.1 shipped+reference-backed bar yet; their registry entries declare a non-`live` `availability` (§9.4.4, RAV-R1). To bring each to reference-backed: implement the handler against a provider, exercise it in the reference implementation, and land conformance vectors. `pay-ap2` additionally needs AP2 provider onboarding (`operator_gated`) — prefer a **neutral processor** (e.g. Adyen / Visa Direct, or Circle/USDC for the stablecoin-backed path) over one operating a competing chain or agent-commerce protocol — and AP2 itself was donated to the FIDO Alliance only in April 2026. |
